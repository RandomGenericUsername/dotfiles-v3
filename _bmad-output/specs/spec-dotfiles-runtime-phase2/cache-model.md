...
## First-run seeding (CAP-5)

Trigger: any runtime command when `current.json` is absent AND `<install>/generated/palettes/colors.yaml` exists (provisioning ran).

1. Hash `<install>/wallpapers/default.png` → `wh`.
2. Create `cache/wallpapers/<wh>/` (hardlink default.png, write meta.json).
3. Hash csg templates dir → template_set_hash; compute `ph`; import `<install>/generated/palettes/colors.*` into `cache/palettes/<ph>/` (write meta.json).
4. Hash effects catalog → catalog_hash; compute `eh`; import `<install>/generated/effects/` into `cache/effects/<eh>/` (write meta.json).
5. Hash icon templates + mappings → templates_hash, mappings_hash; compute `ih`; import `<install>/generated/icons/` into `cache/icons/<ih>/` (write meta.json).
6. Write `current.json` (all four hashes).
7. Create `current/` symlinks.
8. **Repoint consumer paths (R2):** replace the provisioning copies of `~/.config/hypr/colors.conf` and the AGS CSS palette fragment with symlinks → `current/...`, and rewrite hyprpaper conf + ITR color_scheme path to `current/...`. This is the seeder's LAST step — before it, provisioning's pre-runtime copies keep a fresh machine working.
9. Append `history.jsonl` line with `trigger: seed`.

After seeding, runtime WRITES nothing under the install spine. It may still READ spine inputs (templates, catalog, icon templates/mappings) for regeneration — those are part of the cache key.
