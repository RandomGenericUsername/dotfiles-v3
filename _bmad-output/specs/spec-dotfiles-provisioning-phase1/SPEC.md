...
- **Compositor color fragments:** Hyprland skeleton uses `source = ~/.config/hypr/colors.conf` (emitted by CSG's `conf` format, copied from the palette dir); Waybar `style.css` uses `@import "colors.css";` (from CSG's `gtk.css` format — emits `@define-color`, which the Waybar skeleton consumes; review finding 2026-08-12 corrected this from the browser-CSS `css` format); Hyprpaper is flat static pointing at `<install>/wallpapers/default.png`. Phase 2 overwrites only the fragment files.
