...
- [x] Task 1 — filesystem role: spine dirs (AC: 1)
  - [x] `dotfiles/provisioning/filesystem.yaml`: append `- name: config/gtk-3.0` + `- name: config/gtk-4.0` to `entries` after `- name: config/itr` (comment noting the GTK consumer dirs land here in gt-3-1 and host the runtime's `colors.css` pointers at first seed).
  - [x] `src/provisioning/ansible/roles/filesystem/vars/main.yml`: append the same two entries to `filesystem_spine_dirs` in the same order; extend the header comment (config-in-spine list) with the gtk dirs + the runtime-pointer note (provisioning creates the DIRS only — never the `colors.css` pointers, which the runtime seeder owns, gt-2-2).
  - [x] VERIFY NO-EDIT: `src/provisioning/tests/unit/test_filesystem_role.py::test_parity_with_manifest` derives `expected` from the manifest dynamically — manifest + vars updated in lockstep stay green.
- [x] Task 2 — config_links vars (AC: 2)
  - [x] `src/provisioning/ansible/roles/config_links/vars/main.yml`: append `gtk-3.0` + `gtk-4.0` to `config_links_managed_dirs` (AFTER `itr` — list order is parity-locked with `verify_managed_link_dirs`).
  - [x] Same file, new vars: `config_links_gtk_dirs: [gtk-3.0, gtk-4.0]` (the entries that get the migrate-seed + `@import` treatment) and `config_links_gtk_import_line: '@import "colors.css";'`; header comment gains the migrate-then-symlink design note + the nwg-look write-through note (AC 7).
- [x] Task 3 — config_links tasks: migrate-then-symlink inside `_link_one.yml` (AC: 2, 3, 4)
  - [x] `src/provisioning/ansible/roles/config_links/tasks/_link_one.yml`, inserted after the classification task and BEFORE the prompt/backup tasks (exact order: ensure-dir → seed → prompt → backup-mv → append → symlink):
...
- [x] Task 7 — Full green (AC: 6)
  - [x] Record provisioning gate baselines BEFORE coding (gt-2-1/gt-2-2 stash procedure):
    ```bash
...
opencode-go/glm-5.3-flash (opencode, BMad bmad-dev-story run, 2026-09-07)

### Debug Log References

- Baseline gates recorded BEFORE coding (worktree @ 0d30dd4, story pinned `56bde5c`):
  full pytest suite = **4 failed, 533 passed, 3 skipped** (previous interrupted run's baseline, `/tmp/opencode/gt31-baseline3.txt`); ruff check = 3 pre-existing E501 (tests only); ruff format --check = 10 pre-existing unformatted files; `mypy src` = clean.
- Pre-existing BLOCKER found at baseline: `tests/integration/test_settings_parity.py` had a committed SyntaxError (class docstring at line 479 closed with a single `"` instead of `"""`, detected as "unterminated string literal" at line 549) that broke ENTIRE-suite collection. Fixed with a one-character restore of the closing triple-quote (line 479). Without it, the mandated full-suite gate cannot run at all. All other pre-existing failures are unchanged vs the recorded baseline.
- `bootstrap.yaml --syntax-check` fails on `packages.yaml`'s `hosts: "{{ os_family }}"` (needs an inventory providing the seam var) — pre-existing, env-dependent, unrelated to this story; bootstrap structural tests (test_bootstrap_playbook.py) pass.
- Full suite final: **4 failed, 561 passed, 3 skipped** — the failures are EXACTLY the four pre-existing baseline failures (test_settings_parity.py x2 — container/CSG real-apply integration, env-dependent; test_ansible_scaffold.py + test_packages_role.py — packages/group_vars structure). Zero new failures; +28 tests vs baseline.

### Completion Notes List

- Task 1 (filesystem): `config/gtk-3.0` + `config/gtk-4.0` appended to `dotfiles/provisioning/filesystem.yaml` entries and `filesystem_spine_dirs` in lockstep; header comments note the gt-3-1 dirs + the runtime-pointer boundary (dirs only, never the `colors.css` pointers). Parity tests green with zero edits (verified).
- Task 2 (config_links vars): appended `gtk-3.0`/`gtk-4.0` to `config_links_managed_dirs`; new vars `config_links_gtk_dirs` + `config_links_gtk_import_line` (`@import "colors.css";`); header documents migrate-then-symlink (ordering rationale) + nwg-look write-through (AC 7).
- Task 3 (`_link_one.yml`): three gated tasks inserted per the pinned order — ensure spine dir (gtk-gated) → seed copy (`remote_src: true`, trailing-`/`, gated `isdir AND exists AND not correct-link`, BEFORE the mv) → (existing prompt + backup mv untouched) → lineinfile `@import` ensure (no regexp, `create: true`, gtk-gated, NOT classification-gated, AFTER seed + BEFORE symlink) → existing symlink. See "Design deviation 5" below for the one `when`-condition change to the symlink task.
- Task 4 (verify): `verify_install_spine_dirs` + `verify_managed_link_dirs` grew the gtk dirs (both parity tests stayed green with zero edits — verified); new `verify_gtk_skeleton_files` var; new stat+assert pair (follow: true, isreg parity — layer 4) + new grep gate (`grep -E '@import "colors\.css";'` through the `~/.config` links, rc-parity assert, both check-gated); vacuous-pass guard extended with `verify_gtk_skeleton_files | length > 0`; stale "nine spine DIRS" fail_msg wording corrected.
- Task 5 (docs): `docs/02-config-in-spine-pattern.md` gained the "Migrate-then-symlink (GTK consumer dirs, gt-3-1)" subsection (seed-before-move ordering rationale, idempotent `@import` append, nwg-look write-through, dry-run conflict-skip note).
- Task 6 (tests): NEW `tests/unit/test_config_links_role.py` — 24 structural tests (parse/contract/ordering/parity/boundary mirrors of `test_config_copies_role.py` conventions) + 2 real-playbook execution tests (fresh-machine skeleton + idempotency `changed=0`, and migration machine: spine seeding, verbatim snippet preserved with exactly ONE appended import, timestamped backups preserving originals, re-run `changed=0`, import count still 1). `test_verify_role.py` updated: fixture grows gtk spine dirs/symlinks/gtk.css, `_REQUIRED_KEYS` += `verify_gtk_skeleton_files`, new gate/vars/guard tests; parity tests untouched and green.
- Task 7 (gates): ruff check = exactly the 3 pre-existing E501; ruff format = the new test file is clean (1 file reformatted during authoring; suite count unchanged at the pre-existing 10 files); mypy `src` = "Success: no issues found in 17 source files" (note: bare `mypy` without a target errors — pre-existing invocation quirk, recorded in the story's task list as `mypy`; run with `mypy src`). Sanity greps clean: gtk dirs only in filesystem manifest/vars, config_links vars, verify vars (+ tests); `colors.css` in config_links = import line + comments only; no `state_root`/`XDG_STATE_HOME` references.
- VERIFY-ONLY suites all green with zero edits: test_filesystem_role.py, test_bootstrap_playbook.py, test_config_copies_role.py, tests/integration/test_ansible_dryrun.py (after the symlink `when` fix, below), test_verify_role.py (incl. all 5 Runtime execution tests, 4m29s).

### Design deviation 5 — symlink task `when` gains a check-mode conflict-skip (dev decision, documented)

The story pinned "the symlink task is ungated-by-check" and "existing tasks are untouched", but the REAL suite falsified that on this host's machine state: under `--check` the (check-gated) backup `mv` is skipped, so a pre-existing conflicting real dir (this machine's `~/.config/gtk-3.0`) is still on disk when `file state: link` evaluates — and the file module REFUSES to predict over a non-empty real dir even in check mode ("the directory ... is not empty, refusing to convert it", verified against the installed ansible-core: the guard fires before the check-mode mutation skip). This broke `bootstrap.yaml --check` (AC 5 / the dryrun suite's bootstrap case). Fix: the symlink task keeps `not config_links_is_correct_link` and gains `not (ansible_check_mode and config_links_existing.stat.exists)` — an absent target is still predicted (would-change), only the un-predictable conflict case is skipped under --check; real runs are unchanged (mv resolves the conflict, then the link is created — proven by the migration execution test). docs/02's prompting-policy wording already described the link task as check-gated, so the shipped behavior is closer to the spec doc than the pre-change code was; a dry-run note was added to the docs subsection. Structural test `test_symlink_task_ungated_by_check_mode_for_absent_targets` pins the new gating.

### File List

- `dotfiles/provisioning/filesystem.yaml` (modified — manifest entries + comment)
- `src/provisioning/ansible/roles/filesystem/vars/main.yml` (modified — spine dirs + comment)
- `src/provisioning/ansible/roles/config_links/vars/main.yml` (modified — managed dirs + new gtk vars + header notes)
- `src/provisioning/ansible/roles/config_links/tasks/_link_one.yml` (modified — ensure-dir + seed + lineinfile tasks, header notes, symlink `when` check-mode conflict-skip)
- `src/provisioning/ansible/roles/verify/vars/main.yml` (modified — spine dirs, link dirs, new `verify_gtk_skeleton_files`)
- `src/provisioning/ansible/roles/verify/tasks/main.yml` (modified — skeleton stat+assert pair, grep gate pair, guard extension, fail_msg wording)
- `docs/02-config-in-spine-pattern.md` (modified — gt-3-1 subsection)
- `src/provisioning/tests/unit/test_config_links_role.py` (NEW — 26 tests)
- `src/provisioning/tests/unit/test_verify_role.py` (modified — fixture + required keys + 6 new tests)
- `src/provisioning/tests/integration/test_settings_parity.py` (modified — pre-existing SyntaxError fix, one character)
