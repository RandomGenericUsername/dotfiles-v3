...
- [x] Task 2 — Create `adapters/hyprpaper_reloader.py` (AC: 1, 2, 3, 4)
  ...
  - [x] Per-monitor channel call: `subprocess.run([str(hyprctl), "hyprpaper", "wallpaper", f"{monitor},{target}"], …)` — the monitor+path is ONE single-comma-delimited argv element, because hyprctl space-joins argv into the request (`Hyprland/hyprctl/src/main.cpp`: `fullRequest += "{} "`) and hyprpaper's `doWallpaper` splits the RHS ONLY on `,` (`Hyprland/hyprctl/src/hyprpaper/Hyprpaper.cpp` `CVarList2 args(rhs, 0, ',')`). Do NOT pass `monitor` and `path` as separate argv elements (a space between them yields a one-field RHS with no comma → `not enough args`). The path itself may contain SPACES (hyprctl slices RHS after the `wallpaper ` keyword — `rq.find(' ', 12)` — and `doWallpaper` does not space-split, so a space-containing path survives); a COMMA anywhere in monitor or path is NOT representable through this channel (it truncates the comma-split parse) — safe for the default install layout (hash-named cache entries under a standard state_root); document that comma limitation. Pass `capture_output=True, text=True, timeout=10` and wrap every call in the mandated exception tuple `(FileNotFoundError, PermissionError, subprocess.TimeoutExpired, OSError, ValueError)`. The error message is returned on stdout as `error: <detail>` with exit code 1 (hyprctl `makeHyprpaperRequest` → `exitStatus = !result`) — read `result.stdout`/`result.stderr` for the log. Do NOT append a fit-mode field in this story — see Dev Notes "Fit-mode mapping" (hyprpaper applies `cover` by default). (Correction 2026-09-02 review: early draft wrongly claimed spaces are unrepresentable — the comma is the delimiter, verified in Hyprland 0.56.2 `Hyprpaper.cpp`.)
...
- Task 7 green gate: `pytest -q` → **353 passed, 2 skipped** (349 after implementation + 4 review-patch tests; baseline 323 + 30 collected — see arithmetic note); `ruff check` → 3 pre-existing (cli/main.py B008 ×2, domain/models.py E501), 0 new; `ruff format --check` → all 33 files clean; `mypy --strict` → 4 pre-existing, 0 new; `tests/architecture/test_layering.py` → 53 passed.
- Test count arithmetic note (**corrected 2026-09-02 review**): this diff adds **29 new test items** — 25 `test_hyprpaper_reloader.py` unit items (incl. the 5-way parametrized exception case), 3 integration, 1 composition-root wiring test in `test_cli_reconcile.py` — plus the non-test `_PassingHyprpaperReloader` isolation class. Verified suite run 2026-09-02: **353 passed, 2 skipped**. From the 323 baseline that is +30 collected; the extra +1 is a pre-existing crash-recovery variant re-collected in the full run (the original note's mechanism, now confirmed — the earlier "+26"/"25 new" framing was internally inconsistent).
...
- 2026-09-02 — **Code review patch applied (bmad-code-review, D1+D2 resolved + 5 patches):** AC 4 vacuous-True now wins over R5 missing-binary (`reload()` enumerates first; missing `hyprctl` fails only when a symlink exists) with docstring + 3 new hyprpaper unit tests + 2 updated missing-binary tests; composition-root wiring test added (AC 5 finally pinned); docstring/story Dev Notes space-vs-comma claim corrected to the verified comma-delimiter grammar (P1). D1 accepted as R5-literal (DP-1 stub exposure recorded in module docstring + deferred-work). Green gate re-verified: **353 passed / 2 skipped**, ruff 3 pre-existing, format clean, mypy 4 pre-existing, layering 53.
...
**decision-needed**
- [x] [Review][Decision] Composition-root wiring can hard-fail every `reconcile` on a live desktop — the Phase-2 monitor stub always emits `DP-1` (seed_cache.py:154), which most real outputs (eDP-1, etc.) reject (`Invalid monitor`), and a Hyprland host without a running hyprpaper fails too (`failed to connect to hyprpaper`); the crash-recovery Dev Note already logged the live `Invalid monitor` error from the host session. RESOLVED 2026-09-02: **accepted R5-literal surfacing** — the stub is declared Phase-2 scope and real monitor detection is a future story; a stopgap guard would contradict the twice-carried R5 contract. Exposure recorded in module docstring + deferred-work. [cli/main.py:329]
- [x] [Review][Decision] AC 4 vacuous-True vs R5 missing-binary precedence — with no `hyprctl` in PATH and zero `wallpaper-*.png` symlinks, `reload()` returns `False` (binary check runs first) though AC 4 promises `True` for nothing-to-apply; the precedence between the two invariants is unresolved and untested. RESOLVED 2026-09-02: **AC 4 vacuous-True wins** — `reload()` enumerates first and returns `True` for the vacuous state even with no `hyprctl`; missing binary still fails `False` when a symlink exists. Docstring precedence statement + `test_missing_hyprctl_zero_symlinks_returns_true` added; two missing-binary tests updated to seed a symlink so they keep asserting the fail path. [hyprpaper_reloader.py:158]

**patch**
- [x] [Review][Patch] Docstring + FR-6 docs claim spaces in paths are unrepresentable through the IPC — WRONG per the very source cited: hyprctl slices RHS after `wallpaper ` (`rq.find(' ', 12)`) and `doWallpaper` splits ONLY on commas (`CVarList2 args(RHS, 0, ',')`), so a space in the path survives; the genuinely breaking character is a **comma** in the monitor/path. FIXED 2026-09-02: module docstring + story Dev Notes (request grammar) corrected to the comma-delimiter claim; RESOLVED notes in SPINE/SPEC/consumer-wiring were already accurate (channel-only). [hyprpaper_reloader.py:42-46]
- [x] [Review][Patch] AC 5 composition-root wiring has zero positive test coverage — the only real-CLI tests (test_cli_crash_recovery.py) monkeypatch `HyprpaperReloader` to a passing stub; a regression that drops the reloader from `reloaders=[...]` or mis-wires `state_root` passes the suite green. FIXED 2026-09-02: `TestReconcileCompositionRootWiring.test_composition_root_wires_all_reloaders` captures the constructed use case and asserts `[HyprlandReloader, AgsReloader, HyprpaperReloader]` with `state_root` plumbed. [cli/main.py:329]
- [x] [Review][Patch] Dev Agent Record test-count arithmetic is inconsistent — Task 7 claims "349 passed (baseline 323 + **26 new tests**)" while the "Test count arithmetic note" says **25** new test items (22 unit + 3 integration); the actual suite run is 349 passed / 2 skipped (verified this review). FIXED 2026-09-02: Task 7 + arithmetic note rewritten to verified counts (353 passed / 2 skipped; 29 new test items incl. 4 review tests). [rt-2-5-hyprpaper-channel-verification.md:207-208]
- [x] [Review][Patch] Literal AC 4 branch untested — neither vacuous test creates an existing-but-empty `current/` dir; both fall through the `not current_dir.is_dir()` branch, leaving the `if not links` branch dead in tests. FIXED 2026-09-02: `test_current_dir_exists_empty_returns_true` added. [test_hyprpaper_reloader.py:89-101]
- [x] [Review][Patch] `_resolve_hyprctl` explicit separator-path non-executable branch untested — only the `None`→`which` branch is covered; an explicit non-executable path containing a separator (and the `is_file()`-False sub-case) is unprobed. FIXED 2026-09-02: `test_explicit_non_executable_separator_path_fails_later` added. [hyprpaper_reloader.py:112-125]

**defer**
- [x] [Review][Defer] Non-symlink / directory-target `wallpaper-*.png` entries treated as valid monitors — the dangling check only guards missing targets and `is_file()` is never checked on the resolved target; masked upstream by repoint's P2 guard (reconcile.py:205) and stale-cleanup, reachable only by manual tampering. Deferred, pre-existing pattern. [hyprpaper_reloader.py:170-179]
- [x] [Review][Defer] "Hyprpaper reload skipped" log wording undercuts the surfaced-failure it produces — spec-mandated verbatim copy of the Hyprland/AGS sibling reloaders; behavior is correctly `False`. Deferred. [hyprpaper_reloader.py:159]
- [x] [Review][Defer] `_resolve_state_root` duplicates `cli/main.py`'s resolution instead of sharing — spec-directed mirror (Task 2) and the composition root passes `state_root` explicitly; a shared helper is not layering-clean across application/adapters. Deferred. [hyprpaper_reloader.py:88-99]
- [x] [Review][Defer] Failure log concatenates stdout+stderr without a separator and drops a `TimeoutExpired`'s captured output — cosmetic logging, mirrors HyprlandReloader. Deferred. [hyprpaper_reloader.py:195-201]