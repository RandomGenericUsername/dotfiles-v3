---
baseline_commit: 08578e7dadbb4009e3d52c99fce3cf7d696ede8b6
---
...
- [ ] Create `adapters/jinja_template_renderer.py` (AC: 1)
...
- [ ] Create `defaults/templates/colors.json.j2` (AC: 2)
  - JSON output: `{"background": "...", "foreground": "...", "cursor": "...", "colors": [...], "source_image": "...", "backend": "...", "generated_at": "..."}`
- [ ] Create `defaults/templates/colors.sh.j2` (AC: 2)
  - Shell script: `COLOR_BACKGROUND='...'`, etc.
- [ ] Create `defaults/templates/colors.css.j2` (AC: 2)
  - CSS custom properties: `--color-background: ...;`
- [ ] Create `defaults/templates/colors.gtk.css.j2` (AC: 2)
  - GTK CSS: `@define-color color_background ...;`
- [ ] Create `defaults/templates/colors.yaml.j2` (AC: 2)
  - YAML output
- [ ] Create `defaults/templates/colors.rasi.j2` (AC: 2)
  - Rofi RASI
- [ ] Create `defaults/templates/colors.scss.j2` (AC: 2)
  - SCSS variables
- [ ] Create `defaults/templates/colors.sequences.j2` (AC: 2)
  - Terminal escape sequences with `]` and `\` markers for post-processing

### Factory wiring (existing file: factory.py)
- [ ] Add `TemplateDirResolver` to `factory.py`
  - Import and instantiate
  - Add to `CliDependencies`
- [ ] Add `JinjaTemplateRenderer` to `factory.py`
  - Import and instantiate with TemplateDirResolver
  - Add to `CliDependencies`
- [ ] Add `create_template_dir_resolver()` helper function
- [ ] Add `create_template_renderer()` helper function
- [ ] Wire renderer into `CliDependencies` dataclass

### Update LocalProcessor (existing file: local_processor.py)
- [ ] Accept `TemplateRendererPort` in constructor
- [ ] In `process_generate()`: after extraction, iterate `GeneratorConfig.formats` and call `renderer.render()` for each
- [ ] Collect output file paths and populate `GenerationResult.output_files`
- [ ] `process_show()` should NOT render templates (display-only)

### Dependencies
- [ ] Add `jinja2>=3.1` to `pyproject.toml` dependencies

### Write tests
- [ ] Create `tests/unit/adapters/test_jinja_template_renderer.py` (AC: 1, 2, 4, 5)
...
- [ ] Create `tests/unit/adapters/test_template_dir_resolver.py` (AC: 3, 5)
  - test_resolve_uses_env_var_first
  - test_resolve_falls_back_to_xdg
  - test_resolve_falls_back_to_package_defaults
  - test_structural_subtyping
- [ ] Update `tests/unit/adapters/test_local_processor.py` to cover template rendering
...
- [ ] Review all templated Jinja2 content for correctness since templates must produce valid files
...
- [ ] [Review][Defer] TemplateDirResolver custom strategies (DirectoryXdgStrategy, DefaultDirectoryStrategy) extend config-assembler-engine — if config-assembler-engine's CompositePathResolver API changes, these may need updating. This is a pre-existing dependency concern, not specific to this change.
- [ ] [Review][Defer] Sequences binary post-processing uses simple str.replace() — if the template ever contains literal `]` or `\` that should NOT be escaped, this will produce incorrect output. However, the current template design intentionally uses these as markers, so this is by design. Document as a known limitation.
