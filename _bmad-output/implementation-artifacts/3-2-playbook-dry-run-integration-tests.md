...
- [ ] [Review][Decision] AC-2 (become-gated playbooks), AC-3 (apply+verify) and AC-4 (yay-proof) are not demonstrated green anywhere — packages.yaml/bootstrap.yaml are host-skip (become gate) + container-xfail (filed defect), so AC-2 is proven for 8/10 playbooks only; AC-3 always `pytest.skip`s (the `nested_engine_available()` gate is permanently false: `prepare()` installs only uv, `archlinux:latest` ships neither podman nor docker, and the roles document the engine as a runtime dependency they do NOT install — so no host-class can ever pass the gate); AC-4 is host-skip (no root context, yay pre-installed) + container-xfail (filed packages-chain defect). The ten done-criteria and the "makepkg never builds under --check" mutation-free proof are asserted to green by NO test in the suite. Story says "AC 4 is the acceptance gate" — decide whether skip+xfail counts as sufficient for this story or whether the story must remain blocked until the filed defect is fixed and the proofs run green.
- [ ] [Review][Patch] Shared module-scoped container makes the AC-4 test a guaranteed false-negative once AC-3 can run — `test_apply_then_verify_on_disposable_container` is defined first and a real `bootstrap()` installs yay into the same container the AC-4 test reuses; AC-4 then asserts `/usr/bin/yay` absent. The `install_dir=/scratch/data/dotfiles` assertion is equally contaminated (`XDG_DATA_HOME=/scratch/data` → `resolve_install_dir()`). Only shielded today because AC-3 always skips. Fix: separate containers per test (or run the AC-4 proof on a container started AFTER the apply test). [test_apply_verify_container.py:328-348,353-397]
- [ ] [Review][Patch] Honest-gate skip is not applied to the apply+verify test body — `container_target.bootstrap()`/`verify()` raise `_TargetInfraError`/`AssertionError` on filed-defect/network failures instead of `pytest.skip`, so the test ERRORS (not the documented loud skip) on host-classes that pass the nested-engine gate. Wrap the body to convert infra-class failures to loud skips. [test_apply_verify_container.py:255-289,369-370]
- [ ] [Review][Patch] Host-side dry-run suite has no escape hatch for the filed packages-chain defect — `TestDryRunEveryPlaybook[packages.yaml]`/`[bootstrap.yaml]` and `TestYayNeverBuildsUnderCheck` assert `failed=0` with no xfail/skip, so on any root/passwordless-sudo host they hard-fail on the SAME defect the container suite xfails — the two suites disagree about whether packages.yaml --check can pass. [test_ansible_dryrun.py:166-172,189-211]
- [ ] [Review][Patch] Container cleanup is not unconditional — `container_target` fixture calls `cleanup()` only in `except _TargetInfraError` and post-yield teardown; a `subprocess.TimeoutExpired`/`OSError` from `start()`/`prepare()` leaks the `dotfiles-ac3-*` container. Also `podman run` omits the spec'd `--rm`. Fix with try/finally. [test_apply_verify_container.py:340-348,325]
- [ ] [Review][Patch] `_find_repo_root()` deviates from the pyproject-anchored walk-up convention — it matches on `src/provisioning/ansible/ansible.cfg` presence with no `pyproject.toml` guard, so a sibling `src/provisioning` tree in a shared workspace could be matched. [test_apply_verify_container.py:25-33]
- [x] [Review][Defer] Vacuous "clean dry-run" gate for state-asserting playbooks — `verify.yaml`'s asserts are all `when: not ansible_check_mode`-gated, so `verify.yaml --check` performs no work and an all-skipped run passes; the test never asserts `ok>0`/`changed>0`. Spec-explicit design (#51: assert recap exists + failed=0, do NOT assert changed counts); the real coverage is the container apply+verify. [src/provisioning/ansible/roles/verify/tasks/main.yml:138-273]
- [x] [Review][Defer] Recap matching is fragile — `_RECAP_LINE_RE` (`^\s*\S+\s*:.*\bok=\d+.*\bfailed=\d+.*$`) can match stray task-result lines and never validates the expected recap-line count or non-zero `ok`/`changed`; a missing host/play recap line is silently tolerated. [test_ansible_dryrun.py:73,165-174]
- [x] [Review][Defer] `_become_available()` can diverge from ansible's actual become capability — `sudo -n true` succeeds under command-scoped NOPASSWD while the become play may still need a password, producing a wrong skip/run decision. [test_ansible_dryrun.py:129-141]
- [x] [Review][Defer] Killed/timed-out test processes leak the disposable container — cleanup is only in-process; no external sweep for stale `dotfiles-ac3-*` containers. [test_apply_verify_container.py:324-325]
- [x] [Review][Defer] `uv run --directory /repo/...` against the read-only repo mount is brittle — a stale `uv.lock` or any project-dir write fails `prepare()` → skip, and the container-local venv is re-resolved on every module run rather than cached. [test_apply_verify_container.py:83-109,145-175]
