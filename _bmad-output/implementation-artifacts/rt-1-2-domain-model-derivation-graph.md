...
- [ ] Create `src/runtime/src/runtime/domain/models.py` (AC: 1, 2, 3)
  - [ ] Import `dataclasses` (frozen=True, slots=True), `enum.StrEnum`, `typing` only
  - [ ] Use `@dataclass(frozen=True, slots=True)` for all dataclasses (Python 3.14+)
  - [ ] Note: `dict[str, str]` fields (`artifact_hashes`) are mutable inside frozen dataclasses — this is intentional. The dict is a content-addressed hash map populated once at construction; the frozen constraint prevents reassignment of the field, not mutation of the dict contents.
  - [ ] Define `WallpaperEntry` frozen dataclass
    - [ ] `hash_algorithm: str` — always `"sha256"` (required by meta.json)
    - [ ] `kind: str` — always `"wallpaper"` (required by meta.json)
  ...
  - [ ] Define `PaletteEntry` frozen dataclass
    ...
    - [ ] `artifact_hashes: dict[str, str]` — fixed keys: `colors.yaml`, `colors.conf`, `colors.gtk.css` → SHA-256
    - [ ] `generated_at: str` — ISO-8601 UTC timestamp
  - [ ] Define `EffectsEntry` frozen dataclass
    - [ ] `hash_algorithm: str` — always `"sha256"`
    ...
    - [ ] `artifact_hashes: dict[str, str]` — keys are `<filename>.png` → SHA-256
    - [ ] `generated_at: str` — ISO-8601 UTC timestamp
  - [ ] Define `IconsEntry` frozen dataclass
    - [ ] `hash_algorithm: str` — always `"sha256"`
    ...
    - [ ] `artifact_hashes: dict[str, str]` — keys are `<name>.svg` → SHA-256
    - [ ] `generated_at: str` — ISO-8601 UTC timestamp
  - [ ] Define `MonitorWallpaperConfig` frozen dataclass
    - [ ] `backend: BackendType` — StrEnum: hyprpaper, swaybg, swww, mpvpaper
    - [ ] `source_hash: str` — SHA-256 hex of the wallpaper content
    - [ ] `fit_mode: FitMode` — StrEnum: cover, contain, fill, tile, center, stretch (default cover)
  ...
  - [ ] Define `DesktopState` frozen dataclass (projection of current derivation outputs)
  ...
  - [ ] Define `BackendType(StrEnum)` — hyprpaper, swaybg, swww, mpvpaper
  - [ ] Define `FitMode(StrEnum)` — cover, contain, fill, tile, center, stretch
- [ ] Update `src/runtime/src/runtime/domain/__init__.py` (AC: 1)
  - [ ] Re-export all domain models via `__all__` list
  - [ ] `__all__` should include: `WallpaperEntry`, `PaletteEntry`, `EffectsEntry`, `IconsEntry`, `MonitorWallpaperConfig`, `DesktopState`, `BackendType`, `FitMode`
- [ ] Verify layering test passes (AC: 4, 5)
  - [ ] Run `uv run --directory src/runtime pytest`
  - [ ] Confirm domain files use only stdlib allowlist
  - [ ] Confirm no Path FS method calls in domain
