...
- [x] [Review][Decision] Harden install_dir validation or accept the None hole — the verbatim seam assert (`install_dir is defined and install_dir | trim | length > 0`) passes `install_dir=None` (Jinja coerces None → 'None', a 4-char non-empty string), so a mis-invoked `-e install_dir=None` renders `None/generated/palettes` — the exact AC 6 failure the seam exists to prevent. Relative (`install_dir=foo`) and trailing-slash (`/tmp/x/`) values also pass and render non-absolute / `//generated/...` paths. The orchestrator always passes a `.resolve()`d absolute path (use_cases.py:46), so reachability is direct-run operator error only — but hardening the assert would diverge from the verbatim-copy lock shared with 2.5/2.6/2.7/2.9. Fix: strengthen the assert (e.g. `install_dir | type_debug == 'str'` + `is not none` + absolute check) or keep verbatim and accept the gap — **RESOLVED 2026-08-12 (decision: harden)**: assert now a list of five boolean conditions (`is defined`, `is not none`, `is string`, `| trim | length > 0`, `| trim | regex_search('^/') is not none`); the role's assert diverges from the sibling verbatim copy by design (documented in tasks/main.yml header) [tasks/main.yml:35-50]
- [x] [Review][Decision] Resolve the CSG `default_formats` spec contradiction — the rendered settings file bakes `default_formats = ["json", "sh"]` (chaining-spine.md#43), but the default_palette role (2.7) locks `conf, gtk.css, yaml` and documents "NOT the packaged json/sh defaults — no Phase 1 consumer reads colors.json/colors.sh". A bare interactive `csg generate` against the rendered settings emits json+sh the chain cannot consume. Phase 1 uses env overrides so this is latent, but the rendered default contradicts the chain contract. Fix: change the template to `conf, gtk.css, yaml` (deviating from chaining-spine.md#43) or keep json/sh as the tool-default mirror — **RESOLVED 2026-08-12 (decision: change to chain formats)**: template now renders `default_formats = ["conf", "gtk.css", "yaml"]`; runtime test asserts the chain formats [templates/csg-settings.toml.j2:4]
- [x] [Review][Patch] Harden trim-lock test scans with regex — the token `startswith("/")` checks miss quoted literals and glued absolute paths (`"/home/user/x"`, `}}/home/user/generated`); current content is clean, so this only weakens the lock, not the files — **APPLIED 2026-08-12**: `_hardcoded_absolute_path()` regex helper (window-based seam-var whitelist) in test_settings_role.py:48-67 [src/provisioning/tests/unit/test_settings_role.py:280-284, 362-367]
- [x] [Review][Patch] Split the runtime --config gate skip per-tool — `if csg is None or weg is None: pytest.skip` skips the whole gate when either CLI is absent; a machine with only `csg` silently drops the csg half — **APPLIED 2026-08-12**: gate test now skips only when BOTH are absent and gates each present CLI [src/provisioning/tests/unit/test_settings_role.py:562-571]
- [x] [Review][Patch] Add runtime coverage of the XDG fallback branch — both exec tests set `XDG_CONFIG_HOME`, so the real-world default path (`XDG_CONFIG_HOME` unset → `HOME/.config`) is never rendered at runtime; the structural test only checks substrings — **APPLIED 2026-08-12**: new `test_playbook_renders_into_home_config_without_xdg` [src/provisioning/tests/unit/test_settings_role.py]
- [x] [Review][Patch] Add a `--check` run to the exec test — the role claims full check-mode support but no test exercises it; a future `creates:`/gate regression would pass CI — **APPLIED 2026-08-12**: exec test now runs `--check` first, asserts `failed=0` and that no file is written [src/provisioning/tests/unit/test_settings_role.py:475-560]
- [x] [Review][Patch] Assert every `settings_files` dest parent is ensured in `settings_config_dirs` — the two lists are unlinked; a 4th tool entry without a matching dir fails only at runtime ("destination directory does not exist"), never at test time — **APPLIED 2026-08-12**: new `test_every_dest_parent_is_ensured_in_config_dirs` [src/provisioning/tests/unit/test_settings_role.py:325-350]
- [x] [Review][Patch] Scrub ambient `ANSIBLE_*` vars + add `timeout=` on the test subprocess runs — the exec tests copy `os.environ` wholesale, so a developer's `ANSIBLE_INVENTORY`/`ANSIBLE_ROLES_PATH`/etc. can silently alter the run, and a hung ansible-playbook stalls the suite indefinitely — **APPLIED 2026-08-12**: `_test_env()` helper + `timeout=120` on every subprocess.run [src/provisioning/tests/unit/test_settings_role.py:488-497, 576-597]
- [x] [Review][Defer] No fail-loud fact-gathering guard for `ansible_facts.env` — config_copies leads with an `ansible_facts.env.HOME is defined` assert (config_copies/tasks/main.yml:54-60), but the settings first task asserts only `install_dir`; a future 2.12 aggregator (bootstrap.yaml) that forgets `gather_facts: true` dies with an opaque undefined-var error. The settings playbook always sets `gather_facts: true`, so not reachable today — flag at 2.12 build time, mirroring the 2.7-deferred item — deferred, pre-existing [tasks/main.yml:35-41]
- [x] [Review][Defer] `csg info --config` gate is vacuous — csg's `info` wraps resolve in try/except ConfigResolutionError and only echoes a warning, so a parse-broken rendered file still exits 0 (info_cmd.py:43-44); the WEG half can actually fail. The authoritative AC 8 gate is owned by 2.12 (verify) + 3.3 (settings-parity), so this test weakness is not actionable in 2.11 — deferred, pre-existing [tests/unit/test_settings_role.py:562-614]
- [x] [Review][Defer] No schema-parity coverage tying rendered files to the tool schemas — literal string asserts on the .j2 files can't catch a new required schema field; the 3.3 settings-parity integration test invokes the CLIs as the schema gate, so this is future work — deferred, pre-existing [tests/unit/test_settings_role.py:362-430]
- [x] [Review][Defer] `HOME=""` (XDG unset) resolves to `/.config` at the filesystem root — the inner `default()` lacks `boolean=true`, so an empty HOME passes through and dirs get created at `/.config/...`; identical derivation is shared verbatim with filesystem/compositor_configs/config_copies, so the fix must land chain-wide, not here alone — deferred, pre-existing [vars/main.yml:38]
