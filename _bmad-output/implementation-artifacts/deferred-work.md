...
- DECISION (owner, 2026-09-13): stop depending on the SNI menu for the apps we care about and build the menus in AGS instead. Planned shape:
  - Network widget right-click: custom AGS popover — Wi-Fi on/off (`nmcli radio wifi` / AstalNetwork), live list of available networks from `nmcli -t -f SSID,SIGNAL,SECURITY device wifi list` with click-to-connect (in-bar password entry for secured APs), Disconnect, and launch of `wifitui`/`nm-connection-editor`.
  - Tidal right-click: custom AGS popover driven by MPRIS on `org.mpris.MediaPlayer2.tidal-hifi` — Play/Pause, Previous, Next, plus Show/Activate; no reliance on its SNI menu.
  - Unmapped tray apps: keep the current SNI-menu attempt as a fallback.
- Open questions to settle at implementation time: password-prompt UX for secured APs (in-bar entry vs launching `nmtui`/editor), whether to hide the native nm-applet item (already filtered via `isNetworkItem`) permanently, and whether to add a generic per-app custom-menu registry in `status-notifier.ts`. [dotfiles/config/ags/lib/status-notifier.ts, dotfiles/config/ags/bar/widgets/network.tsx, dotfiles/config/ags/bar/widgets/tray.tsx]

## Deferred from: bootstrap failure RCA — full root filesystem aborts pacman -Syy (2026-09-11)

- RECURRING ENVIRONMENTAL CLASS: a root filesystem at 100% makes `pacman -Syy` fail mid-write (`error: failed retrieving file '<repo>.db' ...: Failure writing output to destination, passed N returned M`; rc=1) and aborts `make bootstrap` at `packages : Refresh pacman databases before install`. It is NOT a pacman lock/mirror/keyring problem — diagnose with `df -h /` FIRST. The orchestrator now surfaces the real command error (new `failure_detail` extracted from ansible stdout) and the packages role emits rc/stdout/stderr + likely causes (commit 785f652), so the next occurrence is self-diagnosing. Remediation that unblocked this machine: `sudo rm -f /var/cache/pacman/pkg/*.part`; `sudo journalctl --vacuum-size=200M` (freed ~0.9G); `sudo paccache -rk1` (freed ~3.3G); confirm `df -h /` then re-run. Root fs is small (114G) and `/var/cache/pacman/pkg`, `~/.cache`, and `~/.local/share` grow unbounded — consider a periodic `paccache` + journal cap on this host. [src/provisioning/ansible/roles/packages/tasks/main.yml]
- Proven PRE-EXISTING test failure (present on HEAD b92415a, before the diagnostics patch): `tests/unit/test_packages_role.py::TestPackagesTasks::test_distro_branching_only_in_include_vars` asserts `"ansible_os_family"` appears in exactly two `when`s, but the role uses `ansible_facts['os_family']` (changed in an earlier refactor), so the sum is 0. NFR-3 explicitly permits the Archlinux-fact gate for Arch/pacman logic, so the assertion token is stale — fix the test (or move the refresh gate to a `vars/arch.yml`-loaded var) when the packages role is next touched. [src/provisioning/ansible/roles/packages/tasks/main.yml:46-77, src/provisioning/tests/unit/test_packages_role.py:128-144]
