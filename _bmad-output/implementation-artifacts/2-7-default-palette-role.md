...
- [ ] [Review][Patch] No post-generate verification of output files [src/provisioning/ansible/roles/default_palette/tasks/main.yml:110] — `LocalProcessor._render_formats` collects render errors into `result.stderr` but `process_result` (adapters/output/base.py:24-27) never raises and `main.py` only exits 1 on `ColorSchemeError`, so a template-render failure exits 0 with zero output files and the task reports `ok` (also true in container mode: the inner csg shares this CLI). Add a stat+assert pair (gated `when: not ansible_check_mode`) after generate asserting `colors.conf`/`colors.css`/`colors.yaml` exist in `{{ default_palette_output_dir }}` (AC 2 / fail-loud discipline).
