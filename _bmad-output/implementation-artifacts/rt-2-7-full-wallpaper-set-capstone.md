...
- [x] Task 6 — Integration tests (AC: 2, 3, 4, 5, 6)
  ...
  - [x] **Live-host isolation (at most ONE real-composition exit-code test):** reserve exactly one real-composition `wallpaper set` CLI test for exit-code/R5 evidence, and PROACTIVELY monkeypatch all four reloader classes at their adapter modules (`monkeypatch.setattr("runtime.adapters.hyprland_reloader.HyprlandReloader", _Passing...)`, plus Ags/Hyprpaper/Terminal at the corresponding modules), extending the `test_cli_crash_recovery.py:153-206` isolation pattern — otherwise the command fires the live host's `hyprctl`, `ags`, and `/dev/tty`. NOTE: because the helper does `from runtime.adapters.<x> import <Reloader>` at call time, patching the adapter-module attribute is sufficient (mirrors the existing reconcile isolation). All other E2E coverage composes the use cases directly (bullet above); wiring itself is proven by the Task 5 unit test.

- [x] Task 7 — Stale-scope + full green gate (AC: all)
  - [x] `uv run --directory src/runtime pytest -q` → **≥ 385 passed, 2 skipped** (baseline; expect +new tests from this story)
  - [x] `uv run --directory src/runtime ruff check src/runtime` → zero NEW (3 pre-existing: cli/main.py B008 ×2, domain/models.py E501)
  - [x] `uv run --directory src/runtime ruff format --check src/runtime` → clean (new/edited files)
  - [x] `uv run --directory src/runtime mypy --strict src/runtime` → zero NEW (4 pre-existing)
  - [x] `tests/architecture/test_layering.py` green (changes are cli/application only; no new import categories, no domain/ports edits)
...
- Implemented the capstone as composition-root-only orchestration: `_run_wallpaper_set` now runs `ApplyWallpaperUseCase.run(image_path)` → `ReconcileDesktopStateUseCase.run(trigger="set")` with identical adapters/mutex (double flock held sequentially, AD-12), returning the new module-level frozen `_WallpaperSetResult(apply, reconcile)` dataclass (`src/runtime/src/runtime/cli/main.py`).
- Extracted `_build_reloaders(state_root)` as the single source of the deterministic 4-reloader list `[Hyprland, Ags, Hyprpaper, Terminal]`, used by BOTH `_run_reconcile` and `_run_wallpaper_set` (no duplication).
- Added the `run(trigger: str = "reconcile")` seam to `ReconcileDesktopStateUseCase`, validated fail-loud against the pinned enum `seed|set|reconcile|force` INSIDE `run()` only (ValueError otherwise); `CacheSeeder.append_history` stays trigger-agnostic. Default keeps standalone `reconcile` semantics; `TestReconcileStructuralScopeLock` untouched (`__init__` unchanged).
- `wallpaper set` mirrors the `reconcile` command's reload-failure block exactly: non-empty `reconcile.reload_failures` → `ErrorView(kind="ReloadError")` naming the failed consumer(s) + `typer.Exit(code=1)`; success `CustomView` object now carries `repointed`/`skipped`/`cache_regenerated`/`reload_failures` alongside the existing apply keys, and the one-line summary appends `, N symlink(s) repointed`.
- Refreshed stale scope docs: `reconcile.py` module header now notes the shipped capstone chaining; `apply_wallpaper.py` scope header updated (apply contract itself unchanged); `wallpaper set` command + app help docstrings describe the full end-to-end behavior.
- Tests: unit — updated `_WallpaperSetResult` fakes + JSON object keys, added `test_reload_failures_maps_to_exit_1`, `test_reload_success_renders_repointed_summary`, `TestWallpaperSetCompositionRootWiring.test_composition_root_wires_apply_then_reconcile` (4-reloader wiring + `state_root` + trigger="set" captured), and trigger-param tests in `test_reconcile.py` (4 valid values, default `"reconcile"`, 6 invalid raise ValueError). Integration — NEW `tests/integration/test_wallpaper_set_capstone_integration.py` (7 tests): E2E full pipeline (symlinks/current.json/one "set" history line/each reloader once), cache-hit-on-repeat (zero fake csg/weg/itr calls in BOTH passes, swap+history+reload still run), crash-repair pinning the TRANSIENT `recovery: reverted` INFO log (not just final state), failing-reloader surfaced non-fatally, AD-18 monitors preserved with only `source_hash` moving (DP-1 swww + HDMI-1 mpvpaper), and the AR-9 golden-file guard pinning the exact history line + current.json (timestamps normalized). Exactly ONE real-composition CLI exit-code test with all four reloader classes monkeypatched at their adapter modules (live-host isolation).
- Gates: `pytest -q` → 406 passed, 2 skipped (baseline 385/2 + 21 new tests, zero regressions); `ruff check` → 3 pre-existing (B008 ×2, E501), zero NEW; `ruff format --check` → clean; `mypy --strict` → 4 pre-existing, zero NEW; `tests/architecture/test_layering.py` → 54 passed. No new dependencies, no CLI arguments added, no changes to domain/, ports/, adapters/, seeder.py, pyproject.toml.

### File List

- src/runtime/src/runtime/cli/main.py (modified — composition-root chain, `_build_reloaders`, `_WallpaperSetResult`, reload-failure exit, docstrings)
- src/runtime/src/runtime/application/reconcile.py (modified — `run(trigger=...)` seam + enum validation + docstrings)
- src/runtime/src/runtime/application/apply_wallpaper.py (modified — scope-header docstring refresh only)
- src/runtime/tests/unit/test_cli_wallpaper_set.py (modified — `_WallpaperSetResult` fakes, reload-failure exit, wiring test)
- src/runtime/tests/unit/test_reconcile.py (modified — trigger-param tests)
- src/runtime/tests/integration/test_wallpaper_set_capstone_integration.py (new — E2E capstone integration suite)
- _bmad-output/implementation-artifacts/sprint-status.yaml (modified — story status tracking)

## Change Log

- 2026-09-02 — Story implemented (dev): composition-root apply→reconcile(trigger="set") chain, shared `_build_reloaders` helper, `run(trigger=...)` history-trigger seam with pinned-enum validation, `wallpaper set` reload-failure exit + combined result rendering, stale-scope docstring refreshes, and the full test matrix (unit + E2E integration + golden-file guard + isolated real-composition exit-code test). Suite: 406 passed / 2 skipped; ruff/mypy zero NEW violations.
