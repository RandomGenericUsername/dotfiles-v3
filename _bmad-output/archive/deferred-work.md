...
## Deferred from: code review of 1-2-core-ports-minimal (2026-07-15)

- `@runtime_checkable` does not enforce signatures or method-ness — `runtime_checkable` limitation is a well-known Python trait; real signature enforcement belongs to a static typechecker in CI, which is out of scope for this story.
- Mock `process_generate`/`process_show` use `settings: object` — `AppSettings` doesn't exist yet; will fix when it lands in Story 2.1.
- `OutputPort.error` restricted to `ColorSchemeError` only — broaden to `Exception` if needed in Epics 3 or 4.

## Deferred from: code review of 1-1-domain-foundation (2026-07-15)

- GeneratorConfig.params mutable dict despite frozen dataclass — no current mutator in Epic 1; wrapping in MappingProxyType changes the type contract for downstream consumers. Defer until adapters are introduced.
- GenerationResult.color_scheme typed ColorScheme | None vs spec's non-optional — correct domain design (failure case has no scheme); update spec AC to match code.
