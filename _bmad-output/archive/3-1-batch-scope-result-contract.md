...
### Domain Layer
- [x] Add `ALL` member to existing `ItemType` enum in `domain/enums.py` (use existing EFFECT/COMPOSITE/PRESET — no new enum needed)
- [x] Add `NoInputFilesError`, `BatchProcessingError` to `domain/exceptions.py` if not already present

### CLI Layer
- [x] Create `cli/batch.py` — Typer subcommand group under top-level `app`
  - [x] `app.command("batch")` with subcommands: `effects`, `composites`, `presets`, `all`
  - [x] Common args: `--input`, `--output`, `--flat`, `--explicit-output`, `--strict`, `--parallel`, `--max-workers`, `--runtime`, `--container-engine`
  - [x] Wire batch command context resolution via `_resolve_context` (reuse from `cli/process.py`)
  - [x] Wire `_resolve_processor` for single-item delegation within batch loop
- [x] Register batch subcommand in `cli/main.py` app

### Processor Layer (NEW)
- [x] Create `adapters/batch_processor.py` — `BatchProcessor` adapter
  - [x] Implement `process_batch(BatchRequest) -> BatchResult`
  - [x] Enumerate items: if `item_types` contains `ALL`, expand to all three types
  - [x] For each (item_type, item_name) pair, delegate to `process_effect`/`process_composite`/`process_preset`
  - [x] Collect `ProcessingResult` per item
  - [x] After all items, build `BatchResult(total, succeeded, failed, results, output_dir)`
  - [x] Strict mode: stop on first failure, return partial results
  - [x] Parallel mode: use `concurrent.futures.ThreadPoolExecutor` with `max_workers`
  - [x] Sequential mode: loop in order
  - [x] Handle keyboard interrupt gracefully (partial results)
  - [x] Validate `input_path` exists before enumeration (raise `NoInputFilesError` if missing)
  - [x] No input files matched → return `BatchResult(total=0, succeeded=0, failed=0, results=(), output_dir=...)`

### Factory Wiring
- [x] Add `create_batch_processor(processor, settings, catalog) -> BatchProcessor` in `factory.py`
- [x] Wire `CliDependencies` with optional `batch_processor` field (or compose at command site)

### Output Layer (already implemented — verify)
- [x] Verify `JsonOutputAdapter.batch_result` exists and renders all fields
- [x] Verify `RichOutputAdapter.batch_result` renders batch table
- [x] Verify `PlainOutputAdapter.batch_result` renders summary + per-item lines

### Tests
- [x] Unit tests for `ItemType.ALL` enum value and expansion logic
- [x] Unit tests for `BatchProcessor.process_batch`:
  - [x] All effects processed when scope=EFFECTS
  - [x] All composites processed when scope=COMPOSITES
  - [x] All presets processed when scope=PRESETS
  - [x] ALL expands to effects + composites + presets
  - [x] Partial failure returns mixed outcomes
  - [x] Total failure returns valid BatchResult with succeeded=0
  - [x] Strict mode stops on first failure
  - [x] Parallel mode executes with max_workers
  - [x] No input files → BatchResult(total=0)
  - [x] Input path does not exist → NoInputFilesError
- [x] CLI integration tests via `CliRunner`:
  - [x] `batch effects --input <file>` with populated catalog
  - [x] `batch composites --input <file>`
  - [x] `batch presets --input <file>`
  - [x] `batch all --input <file>`
  - [x] `--strict` stops on failure
  - [x] `--parallel` with --max-workers
  - [x] `--output-format json` produces valid JSON
  - [x] Invalid scope or missing input returns error
...
- [x] [Review][Patch] `BatchResult.total` exceeds `succeeded+failed` in strict/interrupt mode — added `attempted` field to BatchResult
- [x] [Review][Patch] Parallel strict mode doesn't stop item execution — added `executor.shutdown(wait=False, cancel_futures=True)`
- [x] [Review][Patch] Asymmetric exception barrier — added try/except in `_process_sequential` matching parallel pattern
- [x] [Review][Patch] `signal.signal()` unsafe on non-main thread — added `threading.current_thread()` guard
- [x] [Review][Patch] `--runtime`/`--container-engine` CLI args accepted but never used — removed from all 4 batch commands
- [x] [Review][Patch] Stale output directory created before `_resolve_context()` — kept mkdir after resolve (already correct after dead arg removal)
- [x] [Review][Defer] SIGINT handler cannot preempt blocking `future.result()` — non-trivial fix, low impact [`adapters/batch_processor.py:140-141`]
- [x] [Review][Defer] Result ordering non-deterministic in parallel vs sequential — documented behavior tradeoff [`adapters/batch_processor.py:132`]
- [x] [Review][Defer] Default /tmp output directory is world-readable — no cleanup mechanism, pre-existing pattern [`cli/batch.py:38,77,116,155`]
- [x] [Review][Defer] 4x CLI command copy-paste (effects/composites/presets/all) — pre-existing pattern from `cli/process.py` design [`cli/batch.py`]
- [x] [Review][Defer] Help text says "or directory" but `_resolve_context` rejects directories — pre-existing constraint [`cli/batch.py:27,78,129,180`]
