...
| T11 | Frozen glyphs despite a firing epoch: reading through an INNER computed (`defaultSpeakerVolume()`) stays stale even when the outer epoch invalidates, because gnim memoizes each computed on its own deps (measured: live node 0.85, accessor stuck at 0.75). | Inside epoch-tracked computeds, read live GObject props DIRECTLY (`nodeOf(defaultSpeaker())?.volume`), never through another computed. GObject reads are always live; computed caches are not. `useEndpointEpoch` lives in `state.ts` with `endpointEpochStats` diagnostics (`runs`/`subscribes`/`bumps`) exposed via `ags request audio-debug`. |
