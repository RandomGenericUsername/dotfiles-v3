...
- **gt-4-1 amendment (2026-09-08, pointers spec'd as a class):** the exception is now a declarative ConsumerPointer table (gt-2-2, pinned in `shared-data-contract.md`) — AGS → `current/colors.gtk.css`, gtk-3.0 → `current/colors.gtk.css`, gtk-4.0 → `current/colors.adw.css`. One adapter loop implements the shared rules (null-palette removal, file-replace, missing-parent skip, never-dangling). The GTK two-channel mechanism (`colors.adw.css` named colors + `:root` variables) and the `GTK_THEME` env hazard are recorded in `consumer-wiring.md`.
...
- **Real monitor detection (Phase-2 stub):** seeding and apply previously defaulted to a single hardcoded `DEFAULT_MONITOR = "DP-1"` (`seed_cache.py`, `apply_wallpaper._build_monitors`), which the hyprpaper reloader then fed to `hyprctl hyprpaper wallpaper DP-1,…` — rejected on real desktops whose outputs are named e.g. `eDP-1`/`HDMI-A-1` (`Invalid monitor`). Originally accepted R5-literal behavior. SHIPPED 2026-09-02 (rt-2-7 review): new port `IMonitorSource` + adapter `HyprlandMonitorSource` (`hyprctl monitors -j`; enabled, non-mirrored outputs; deterministic order; `[]` on any detection failure) injected at the composition root into `SeedCacheUseCase` and `ApplyWallpaperUseCase`. `DEFAULT_MONITOR`/`DP-1` remains ONLY as the fallback when no source is injected or detection is empty (headless/CI). `ReconcileDesktopStateUseCase` reads stored monitors and is untouched (scope lock).
