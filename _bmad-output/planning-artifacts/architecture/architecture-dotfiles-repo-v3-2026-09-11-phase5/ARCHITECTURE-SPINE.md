...
- **Rule:** one **shared, machine-enforced, versioned** contract on the session bus, pinned by `contracts/event-contract.md` + `contracts/event-contract.json` (names **and** schemas). The hub is **broker + job registry** and the sole emitter. Interface (`org.dotfiles.Events1`) methods: `Emit(topic,payload)`, `BeginJob(kind,ttl)→job_id`, `RenewJob(job_id)`, `AdoptJob(job_id,pid)`, `ReportProgress(job_id,fraction)`, `EndJob(job_id,exit_code)`, `Control(job_id,action)`, `GetActiveJobs`, `GetTopicState(topic)`. Signals: `JobStarted`/`JobProgress`/`JobFinished`, `DomainEvent`, `JobsCleared`; every signal carries the hub **`epoch`**, and `JobsCleared(epoch)` is emitted on **first start and every restart** (a consumer also treats D-Bus `NameOwnerChanged` as a restart signal). The hub allocates `job_id`; a job holds a **lease** (`ttl` + `RenewJob`) so a job that dies without `EndJob` is expired by the hub (synthetic `JobFinished`). Method failures are typed D-Bus errors (`UnknownJob`, `JobEnded`, `UnknownTopic`, `PayloadTooLarge`, `RateLimited`, `NotControllable`). The runtime implements its side behind a port (`IEventPublisher`/`IEventSubscriber`) + D-Bus adapter; the core never imports D-Bus. The bar is a thin consumer. Version is in the interface name: an additive optional field, or a new method/signal/topic, is **not** breaking; a removed/retyped/newly-required field is `…Events2`, and `Events1`/`Events2` may coexist on the bus under distinct interface names during migration. `seq` is per-topic monotonic **within an `epoch`**; consumers compare the `(epoch, seq)` pair. [ADOPTED]
...
- **Rule:** the daemon reacts **only** to the watched root set (AD-39); that set is file-wise disjoint from everything the runtime writes. "Converge" is the composite **`CheckInputs → RegenerateStale → Reconcile → declarative converge`** (a spine-input change must re-derive, not merely repoint). The **backstop** is a last-converged hash of the four derivation inputs **plus the intent document**, persisted under `state_root` (an output location, never a watched root): on any event and **on start** (after readiness, non-gating — AD-33), recompute it; if unchanged, do nothing; else converge and persist. This makes the daemon idempotent and repairs downtime drift. An **unseeded** runtime (no `current.json`) is a benign no-op — never an error, never a restart; the daemon never seeds (AD-11). The runtime never writes a watched root. [ADOPTED]
...
- **Rule:** classify by lifetime and who knows the state:
  ...
  - **Lifetime jobs** (capture recorder, speed test) own the observed process (ownership is **transitive**: a wrapper may **adopt** the observed child via the hub, recording its PID/owner, so the wrapper can exit without orphaning it) and **report via hub methods** (`BeginJob`/`RenewJob`/`ReportProgress`/`EndJob`) — the **hub** emits the lifecycle signals. A job that exposes control registers its allowed actions with the hub; the hub is the **single control path** (`Control(job_id, action)`) — a UI never shells the tool directly.
...
- **Rule:** inotify over the AD-39 roots only; **coalesce/debounce** bursts into a single reconcile; on **`IN_Q_OVERFLOW`** perform a full recursive re-scan of the affected root. **Watch-registration loss** — `IN_IGNORED`, `IN_MOVE_SELF`, a moved/renamed root, or `max_user_watches` exhaustion — re-establishes the watches and performs a full re-scan; a root on a filesystem without inotify is unsupported and surfaced, never silently left unwatched. A directory watch persists across in-directory atomic replaces (a file-root watch via the immediate parent outlives the replace). **No polling** (state is event-sourced; timers render only continuous values); the tree watches are installed explicitly per level to the bounded depth. [ADOPTED]
...
- **Rule:** automatic convergence ships **observe-only first**; status is `systemctl --user` plus a read-only inspect surface; a kill switch stops the unit; every automatic action (regenerate/prune) is logged with its trigger; the prune delete-audit line is retained; `doctor` detects and repairs `current.json`↔history divergence (a required capability, not yet implemented — see remediation R‑5). Failures are routed **fatal vs recoverable**: a corrupt/partial state or `ENOSPC` is recoverable (logged, retried with backoff, surfaced) and must never crash-loop the unit. The daemon assumes its own last write may have failed and re-checks rather than trusting it. [ADOPTED]
...
- **Rule:** each shared contract has **exactly one machine-checkable definition**, and conformance is checked by **executing** a script — never by reading prose and asserting compliance on each side. The definition is per contract type:
  - **D-Bus wire** (names, methods, signals, type signatures): one **introspection XML** (`contracts/event-contract.xml`), parsed/enforced by GJS natively (`Gio.DBusNodeInfo`) and by Python via a declared signature table + assertion; payload dictionary shapes/enums live in a **JSON Schema** referenced from the XML.
...
| Locks | use cases own `.seed.lock` / `.history.lock`; the daemon acquires **no lock across a use-case call** (flock is non-reentrant) |
