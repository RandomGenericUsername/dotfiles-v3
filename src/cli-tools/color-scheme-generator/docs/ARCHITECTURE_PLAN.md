...
| `TemplatesSerializer` | `dump-templates` helper | Copies bundled `defaults/templates/*.j2` into `$XDG_CONFIG_HOME/color-scheme-generator/templates/` (mkdir parents). `--overwrite` required to replace existing |
