...
_config = GeneratorConfig(
    ...
    params={"saturation": 1.0, "algorithm": "kmeans"},
...
class TestWallustGenerator:
    def test_is_available_returns_true_when_wallust_on_path(self):
        gen = WallustGenerator()
        with patch("shutil.which", return_value="/usr/bin/wallust"):
            assert gen.is_available() is True

    def test_is_available_returns_false_when_wallust_not_on_path(self):
        gen = WallustGenerator()
    ...
    def test_generate_raises_backend_not_available_when_wallust_missing(self):
        ...
        with patch.object(gen, "is_available", return_value=False):
            with pytest.raises(
                BackendNotAvailableError, match="Install wallust binary"
            ):
    ...
    def test_generate_parses_cache_correctly(self):
        ...
        assert scheme.backend == Backend.WALLUST
        ...
        with patch.object(gen, "is_available", return_value=True):
            with patch("subprocess.run") as mock_run:
                ...
                with patch.object(gen, "_parse_stdout", return_value=_16_colors):
                    cfg = GeneratorConfig(
                        backend=Backend.WALLUST,
                        params={"saturation": 0.5, "algorithm": "kmeans"},
                        formats=(ColorFormat.JSON,),
                        output_dir=Path("/tmp/output"),
                    )
    ...
    def test_generate_raises_color_extraction_error_on_nonzero_exit(self):
        ...
        assert exc.value.backend == Backend.WALLUST
    ...
    def test_generate_raises_color_extraction_error_on_timeout(self):
        ...
        with patch.object(gen, "is_available", return_value=True):
            with patch(
                "subprocess.run",
                side_effect=subprocess.TimeoutExpired(cmd=["wallust"], timeout=60),
            ):
                with pytest.raises(ColorExtractionError) as exc:
                    gen.generate(Path("/tmp/test.png"), _config)
