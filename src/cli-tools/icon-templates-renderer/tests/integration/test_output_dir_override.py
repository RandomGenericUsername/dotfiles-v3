...
class TestOutputDirOverride:
    def test_outputs_root_drives_output_location(self, cli_deps_integration, tmp_path):
        ...
        outputs_root = tmp_path / "output-root"
        ...
        assert (outputs_root / "battery" / "battery-0.svg").exists()

    def test_cli_output_dir_override(self, cli_deps_integration, tmp_path):
        colors = tmp_path / "colors.yaml"
        colors.write_text("special:\n  background: '#000000'\ncolors: []\n")
        template_dir = tmp_path / "templates" / "battery"
        template_dir.mkdir(parents=True)
