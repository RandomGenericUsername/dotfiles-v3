...
_BINARY_CANDIDATES = ["docker", "podman"]


class OCICommandRunner(CommandRunnerPort):
    def __init__(
        self, engine: str | None = None, sanitizer: CommandSanitizer | None = None
    ) -> None:
        self._sanitizer = sanitizer or CommandSanitizer()
        self._binary = engine or self._detect_binary()
