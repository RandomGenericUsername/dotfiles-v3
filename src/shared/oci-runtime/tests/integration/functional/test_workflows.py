...
def _inject_responses(transport, responses):
    for key, val in responses.items():
        if isinstance(key, str):
            key = tuple(key.split())
        transport._responses[key] = val


def _inject_stream_responses(streaming, responses):
    for key, val in responses.items():
        if isinstance(key, str):
            key = tuple(key.split())
        streaming._responses[key] = val


def _inject_stream_chunks(streaming, responses):
    for key, val in responses.items():
        if isinstance(key, str):
            key = tuple(key.split())
        streaming._stream_responses[key] = val
