...
class ContainerManagerContractTest(ABC):
    @abstractmethod
    def make_manager(self, transport: Transport, parser: ContainerParser, caps: RuntimeCapabilities) -> ContainerManager:
        ...

    def _defaults(self):
        t = RecordingTransport("docker")
        caps = RuntimeCapabilities()
        return self.make_manager(t, MockContainerParser(), caps), t

    def _failing(self, stderr="No such container: nonexistent"):
        t = FailingTransport("docker", stderr=stderr)
        return self.make_manager(t, MockContainerParser(), RuntimeCapabilities()), t
    ...
    def test_run_returns_str(self):
        ...
        t._responses["docker run"] = ExecResult(0, b"abc\n", b"")
...
class TestCliContainerManagerContract(ContainerManagerContractTest):
    def make_manager(self, transport, parser, caps) -> ContainerManager:
