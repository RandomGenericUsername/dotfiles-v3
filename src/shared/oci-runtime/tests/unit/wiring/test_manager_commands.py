...
INSPECT_NETWORK_JSON = (
    b'[{"Id":"net123","Name":"bridge","Driver":"bridge","Scope":"local","Labels":{}}]'
)
LIST_NETWORK_JSON = (
    b'[{"Id":"net123","Name":"bridge","Driver":"bridge","Scope":"local","Labels":{}}]'
)
...
class TestImageManagerCommands:
    ...
    def test_list_with_filter(self, t, caps):
        t._responses = {
            ("docker", "image", "list", "--filter", "label=app=web"): RawExecResult(
                0, b"[]", b""
            )
        }
        mgr = _image_mgr(t, DockerImageParser(), caps)
...
class TestContainerManagerCommands:
    ...
    def test_run_detach_false(self, t, st, caps):
        st._responses = {
            ("docker", "run", "--network", "bridge", "alpine"): RawExecResult(
                0, b"abc123", b""
            )
        }
        mgr = _container_mgr(t, DockerContainerParser(), caps, streaming=st)
    ...
    def test_run_podman_flags(self):
        ...
        podman_t = RecordingTransport(
            "podman",
            {
                "podman run --userns=keep-id -d --network bridge alpine": RawExecResult(
                    0, b"abc123", b""
                )
            },
        )
        podman_st = RecordingStreamingTransport(
            "podman",
            {
                "podman run --userns=keep-id -d --network bridge alpine": RawExecResult(
                    0, b"abc123", b""
    ...
    def test_list_with_filter(self, t, st, caps):
        t._responses = {
            ("docker", "container", "list", "--filter", "name=web"): RawExecResult(
                0, b"[]", b""
            )
        }
        mgr = _container_mgr(t, DockerContainerParser(), caps, streaming=st)
    ...
    def test_exec(self, t, st, caps):
        t._responses = {
            ("docker", "exec", "ctr1", "ls", "-la"): RawExecResult(
                0, b"file1\nfile2\n", b""
            )
        }
        mgr = _container_mgr(t, DockerContainerParser(), caps, streaming=st)
    ...
    def test_exec_detach_user(self, t, st, caps):
        t._responses = {
            ("docker", "exec", "-d", "-u", "root", "ctr1", "ls"): RawExecResult(
                0, b"", b""
            )
        }
        mgr = _container_mgr(t, DockerContainerParser(), caps, streaming=st)
...
class TestImageManagerBuildCommands:
    def test_build_with_no_cache_and_target_flags(self, transport, streaming, caps):
        ...
        transport._responses = {
            (
                ...
                "stage1",
                "-",
            ): RawExecResult(0, b"sha256:abc123def456\n", b"")
        }
        mgr = _image_mgr(transport, DockerImageParser(), caps)
        ctx = BuildContext(
            build_file_content="FROM alpine", no_cache=True, target="stage1"
        )
    ...
    def test_build_with_file_path_and_context_path(self, transport, streaming, caps):
        ...
        mgr = _image_mgr(transport, DockerImageParser(), caps)
        ctx = BuildContext(
            build_file_path=Path("/d/Dockerfile"), context_path=Path("/ctx")
        )
        mgr.build(ctx, "myimg", timeout=30)
        cmd = transport.calls[-1].command
        assert "-f" in cmd
...
class TestVolumeManagerCommands:
    ...
    def test_list_with_filter(self, t, caps):
        t._responses = {
            ("docker", "volume", "list", "--filter", "label=app=web"): RawExecResult(
                0, b"[]", b""
            )
        }
        mgr = _volume_mgr(t, DockerVolumeParser(), caps)
...
class TestNetworkManagerCommands:
    ...
    def test_connect(self, t, caps):
        t._responses = {
            ("docker", "network", "connect", "mynet", "ctr1"): RawExecResult(
                0, b"", b""
            )
        }
        mgr = _network_mgr(t, DockerNetworkParser(), caps)
    ...
    def test_disconnect(self, t, caps):
        t._responses = {
            ("docker", "network", "disconnect", "mynet", "ctr1"): RawExecResult(
                0, b"", b""
            )
        }
        mgr = _network_mgr(t, DockerNetworkParser(), caps)
    ...
    def test_disconnect_force(self, t, caps):
        t._responses = {
            ("docker", "network", "disconnect", "mynet", "ctr1", "-f"): RawExecResult(
                0, b"", b""
            )
        }
        mgr = _network_mgr(t, DockerNetworkParser(), caps)
