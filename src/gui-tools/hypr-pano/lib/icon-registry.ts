// Clipboard icon resolver — consumes the SAME rendered-icon pipeline as the bar.
//
// Templates live in the repo (`dotfiles/assets/icon-templates/clipboard/`),
...
// <spine>/config/ags/icons.json, exposed at ~/.config/ags). The overlay reads
...
function loadManifest(): IconManifest {
  ...
  try {
    ...
    manifest = ok && bytes !== null ? (JSON.parse(new TextDecoder().decode(bytes)) as IconManifest) : {}
  } catch (error) {
    console.error(`hypr-pano: cannot read icon manifest: ${error}`)
    manifest = {}
  }
  return manifest
}
