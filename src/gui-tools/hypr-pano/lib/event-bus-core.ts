// Pure core for the hypr-pano clipboard overlay's `org.dotfiles.Events1`
// consumer (Phase 5, AD-34/AD-37/AD-40). Copied from the bar's proven core
// (`dotfiles/config/ags/lib/event-bus-core.ts`) — a standalone AGS app cannot
// import across config directories — and extended with the clipboard topics.
//
...
// import and exercise it (parity with `runtime/adapters/bar_subscriber.py`).
// `event-bus.ts` supplies the Gio transport.
//
// Contract literals are pinned to `contracts/event-contract.json` / `.xml` by
// `src/gui-tools/hypr-pano/tests/event-contract-drift.mjs` (executed, never
// text-scanned).
...
export const CAPTURE_STATE_TOPIC = "capture.state";
export const SPEEDTEST_FINISHED_TOPIC = "speedtest.finished";
export const CLIPBOARD_UPDATE_TOPIC = "clipboard.update";
export const CLIPBOARD_STATE_TOPIC = "clipboard.state";
...
//: cannot stall the bar at start-up.
...
export class DomainEventBusCore {
  ...
  //: handlers, so a bar that starts mid-recording renders the current state
  //: without waiting for the next transition/tick.
  ...
  //: the UI first, then re-read every tracked topic so a still-live capture
