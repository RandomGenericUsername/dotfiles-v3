// Pure core for the wallpaper selector's `org.dotfiles.Events1` consumer.
// Copied from the hypr-pano proven core
// (`src/gui-tools/hypr-pano/lib/event-bus-core.ts`) — a standalone AGS app
// cannot import across config directories — and extended with the
// `wallpaper.state` topic.
//
...
// `src/gui-tools/wallpaper-selector/tests/event-contract-drift.mjs`
...
//: cannot stall the selector at start-up.
...
export class DomainEventBusCore {
  ...
  //: handlers, so a selector opened mid-apply renders the current state
