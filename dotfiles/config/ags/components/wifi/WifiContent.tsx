...
/**
 * Embeddable Wi-Fi body (network list + password prompt) shared by the settings
 * panel subview and the bar popup.
 *
...
/** Whether the password prompt is on screen — drives the window keymode. */
...
function wifiIconOn(): string | null {
  return registry.resolve("settings-panel", "wifi")
}

function wifiIconOff(): string | null {
  return registry.resolve("settings-panel", "wifi-off")
}

/** Header/standalone power toggle for the Wi-Fi content. */
...
export function WifiContent({ visible }: { visible: Accessor<boolean> }) {
  // Scan while shown; the service owns the cadence and pauses while a connect
  // is in flight. The service refcounts so panel and popup never fight.
  createEffect(() => {
    if (!visible()) return
    startScanning()
    return () => stopScanning()
