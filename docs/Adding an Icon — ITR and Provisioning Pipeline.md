...
> **Epic 4 amendment (2026-09-04, single source of truth):** the
> palette/icons are derived by the runtime (`wallpaper set` / first-run
> seed) into `cache/<layer>/<hash>/` and served through `current/`
> symlinks. There is no `generated/` tree and no provisioning render
> step. The diagram below is updated; `generated/` references elsewhere
> in this guide that describe the old provisioning render are historical.
...
flowchart LR
    ...
    PALETTE["Runtime palette<br/>current/colors.yaml"]
    ASSETS["Ansible assets role<br/>install spine"]
    SETTINGS["ITR settings.toml<br/>templates + color scheme + output"]
    SEED["runtime seed / wallpaper set"]
    ITR["itr render (via runtime)"]
    CACHE["Cached SVG<br/>cache/icons + current/icons"]
    JSON["AGS manifest<br/>config/ags/icons.json"]
    REGISTRY["AGS IconRegistry (current/ only)"]
    WIDGET["AGS widget"]
...
1. The `assets` role deploys icon templates and icon mappings into the install
   spine.
2. The settings workflow creates the ITR `settings.toml` (pointing at
   `current/colors.yaml`).
3. The runtime seed (`wallpaper set <install>/wallpapers/default.png`,
   run by the `runtime-seed` bootstrap step) derives palette + icons.
4. ITR reads the deployed mappings, templates, vocabulary, and the
   runtime palette.
5. ITR writes rendered SVGs to `cache/icons/<ih>/`, served through
   `current/icons/`.
6. The compositor configuration role deploys AGS source files, including
   `icons.json` and `icon-registry.ts`.
7. AGS resolves the runtime file (`current/icons/`, no fallback) and
   loads it in the widget.
...
| Runtime palette | Derived by the runtime seed / `wallpaper set` | `~/.local/state/dotfiles/current/colors.yaml` |
| Rendered icons | ITR output (via runtime) | `~/.local/state/dotfiles/current/icons/` |
...
| `variants[].output` | Filename written under `current/icons/` (runtime cache entry) |
...
shape, pick a palette swatch, choose the scope, save. The authoring model
is repo-authoritative: the editor targets the repo manifest
(`dotfiles/config/icon-template-color-scheme-mappings/`) whenever a
checkout exists — the provisioned launcher (`icon-color-mapping-editor`,
`SUPER+I`) `cd`s into the checkout — and the next bootstrap converges the
manifest into the spine, so "edit repo, bootstrap, deployed" is the whole
workflow. On machines without a checkout the editor edits the spine copy
instead. From a checkout you can also run it with `make run` inside
`src/gui-tools/icon-color-mapping-editor`. The editor previews the
group-wide effect before writing and never touches templates, the palette,
or `generated/`.
...
5. Otherwise return `null` (missing icons are visible as empty images —
   never silently stale; Epic 4 removed the `generated/` fallback).
...
The runtime seed / `wallpaper set` consumes (Epic 4 — the
provisioning `icons` role is deleted):

```text
<install_dir>/icon-mappings/icons.yaml
<install_dir>/icon-templates/
<state>/dotfiles/current/colors.yaml
<install_dir>/config/itr/settings.toml
```

It invokes (via the runtime, same args):
...
itr render \
  ...
  --icon screen-recorder \
  --template-dir dotfiles/assets/icon-templates \
  --color-scheme "$HOME/.local/state/dotfiles/current/colors.yaml" \
  --output-dir "$tmpdir"
...
The runtime seed should report icons generated (or cache hit), and the
verify role should report that `current/icons/` holds the rendered SVGs.
...
```bash
ls -l "$HOME/.local/state/dotfiles/current/icons/screen-recorder-"*.svg
grep -h -o 'fill="#[0-9a-fA-F]*"' \
  "$HOME/.local/state/dotfiles/current/icons/screen-recorder-"*.svg \
  | sort -u
```
...
- [ ] `current/icons/` contains each rendered output (after `wallpaper set`).
