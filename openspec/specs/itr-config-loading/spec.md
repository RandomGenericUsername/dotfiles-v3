# itr-config-loading Specification

## Purpose
Defines how the icon-templates-renderer (ITR) loads configuration: a unified settings-first path precedence for templates, color scheme, and output directories, and a simplified icons YAML shape with relative per-group dirs and no top-level roots or per-group color scheme.
