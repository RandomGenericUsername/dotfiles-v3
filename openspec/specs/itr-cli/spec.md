# itr-cli Specification

## Purpose
Defines the icon-templates-renderer (ITR) command-line surface: optional path flags, a `--config` option selecting `settings.toml`, fail-fast errors when required roots are missing, and a `list` command that works without resolved path roots.
