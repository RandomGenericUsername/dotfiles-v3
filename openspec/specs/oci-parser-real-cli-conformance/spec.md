...
- **THEN** `info.labels` is `{}` (empty dict), not `None`
...
- **WHEN** `PodmanImageParser.parse_id_from_pull()` is fed real podman pull output that contains a bare hex id
...
- **THEN** the resulting domain object's `labels` field is always a `dict`, never `None`
