...
WEG's test seam is an env-var + module-global pair (`WEG_TEST_DEPS` env var checked on every production invocation, `_test_deps` module-global + `set_test_deps` helper) that leaks test state across tests and carries test-only machinery in production. CSG validated the cleaner pattern: a single `build_deps()` factory the callback calls, with tests injecting fakes via `monkeypatch.setattr`. This spec defines the replacement seam and the regression guards that enforce it.
