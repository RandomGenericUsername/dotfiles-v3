...
`--hairline → alpha(@color_foreground, .12)`, `--accent → @color_06` for BOTH
mode accents and both primary CTAs (one accent; modes differ by icon + record dot).
Caution/severity (settings errors) → `@color_03`.
