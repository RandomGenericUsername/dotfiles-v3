# CSG Output Port Routing

## Why

`install`, `uninstall`, `version`, and `list-backends` rendered user-facing output through `isinstance(adapter, JsonOutput/RichOutput/PlainOutput)` dispatch and adapter-private methods (e.g. `print_table`), bypassing the `OutputPort` abstraction. An adapter implementing only the port interface would produce nothing from these commands. This capability pins the corrected contract.
