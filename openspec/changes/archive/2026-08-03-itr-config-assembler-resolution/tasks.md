...
- [x] 2.4 `tests/unit/domain/` tests for the above

## 3. Ports & adapters: resolvers

- [x] 3.1 `ports/template_dir_resolver.py`: `TemplateDirResolverPort.resolve() -> Path | None`
- [x] 3.2 `ports/color_scheme_resolver.py`: `ColorSchemeResolverPort.resolve() -> Path | None`
- [x] 3.3 `adapters/template_dir_resolver.py`: discovery-only `CompositePathResolver`; `PathResolutionError` → None
- [x] 3.4 `adapters/color_scheme_resolver.py`: discovery-only; `PathResolutionError` → None
- [x] 3.5 tests; `tests/unit/ports/test_contracts.py` covers both

## 4. Settings resolver (real)

- [x] 4.1 `adapters/assembled_config_resolver.py`: add OverrideRules (`output.output_dir`, `output.verbosity`, `templates.dir`, `color_scheme.path`); map to `AppSettings(output, templates, color_scheme)`
- [x] 4.2 `defaults/settings.toml`: `[output]` `output_dir` + `verbosity`; `[templates]`/`[color_scheme]` commented
- [x] 4.3 tests: settings/env/CLI/bundled for `output_dir`; `templates.dir`/`color_scheme.path` from settings

## 5. Config loader & icon renderer

- [x] 5.1 `adapters/yaml_icon_config_loader.py`: `load(yaml_path, roots: ResolvedRoots | None = None)`; remove top-level roots popping; per-group join (`root / sub`); remove per-group `color_scheme` resolution (global only)
- [x] 5.2 `adapters/icon_renderer.py`: thread `request.roots` into `load`/`load_one`; `list` tolerates None roots; render/validate require roots
- [x] 5.3 tests: join precedence; None roots behavior

## 6. Composition root & CLI

- [x] 6.1 `factory.py`: `CliDependencies` += `template_dir_resolver`, `color_scheme_resolver`; `create_*` factories; `build_deps` wires all three
- [x] 6.2 `cli/options.py`: `CONFIG_OPT` (`--config`, exists/file/resolve)
- [x] 6.3 `cli/_helpers.py`: `resolve_roots(...) -> ResolvedRoots` (D7); clear `ConfigResolutionError` on missing required root
- [x] 6.4 `cli/main.py`: `build_deps` wires resolvers; app help documents the discovery chains + env override format
- [x] 6.5 `cli/render.py`, `cli/list_cmd.py`, `cli/validate.py`: add `--config`; call `resolve_roots`; build `ResolvedRoots`; pass into request
