...
The container processor adapter constructs a CLI argv it passes into the container. This argv must be parseable by the in-container CLI: if the CLI's flag scope changes (root → sub-typer callback → leaf) and the adapter is not updated, the argv will be rejected at runtime. Existing tests mock the container engine and never validate the argv against the live CLI — assertions like `"process effect blur" in result.command` catch substring presence but not flag positioning. Both `cli-flag-scope-completion` (commit `29883d2`) and `cli-flag-scope-refinement` moved flags between scopes without tests failing.
