# CSG Config Resolution Chain

## Why

The resolver (`adapters/settings/config_resolver.py`) implements a 5-strategy discovery chain:
1. CLI path (`--config`)
2. ENV path (`COLORSCHEME_CONFIG_FILE_PATH`)
3. CWD traversal (up to 3 parent levels, `max_levels=3`)
4. XDG default (`~/.config/color-scheme-generator/settings.toml` or `$XDG_CONFIG_HOME/color-scheme-generator/settings.toml`)
5. Package-bundled defaults

Additionally, `COLORSCHEME__SECTION__KEY` double-underscore naming provides ENV-based field overrides. This chain has zero test coverage today — `test_config_resolver.py` covers resolver invocation but not the priority order or ENV semantics.
