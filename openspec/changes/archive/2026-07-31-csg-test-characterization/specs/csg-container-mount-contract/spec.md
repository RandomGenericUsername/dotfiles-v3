# CSG Container Mount Contract

## Why

The `ContainerProcessor` (`adapters/container_processor.py`) wraps the inner `csg` CLI in an OCI container. Its mount plan, environment, temp-artifact serialization, and error handling are only weakly asserted today — the existing tests use a `Mock()` runtime and assert mount targets but not sources, `_CONTAINER_ENV` fidelity, or the interface contract between the adapter's inner argv and the real CLI parser.
