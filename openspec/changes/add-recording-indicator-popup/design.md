...
- `.rec-dot`: 9px REC-red circle (`#e0605f`, the ONE literal in the capture stack — "REC" must
  read red regardless of wallpaper; the pulse ring shares it) with a 1.6s pulse ring (GTK CSS
  `@keyframes` where supported; else static dot). Paused → `@color_03`, no pulse.
- `.rec-timer`: tabular-nums, bold, min-width so the cluster doesn't jitter as digits change.
- `.rec-btn`: 26px ghost buttons; hover surface; `.stop:hover` tints with the palette caution slot
  (`@color_03`, not a fixed red). Icons from `capture-tool` group (`pause`/`play`/`stop` at 16px)
  via the same `iconPath(state…)` pattern already in the widget.
...
- Actions grid: Pause/Resume + Stop with icons; Stop hover tints with the palette caution slot (`@color_03`, matches mockup `.danger` intent but palette-derived).
