# CSG CLI Integration Tests

## Why

The `csg` CLI command tests are mock-heavy — they patch `build_deps`/`create_local_processor`/`create_container_processor` (in two namespaces: `cli.main` and `cli._helpers`) and assert on mock call counts rather than observable behavior. The test seam is the existing `CliDependencies.processor` slot: commands short-circuit processor construction when a fake is pre-set, letting tests drive the real Typer `app` via `CliRunner` with a fake at the port boundary.
