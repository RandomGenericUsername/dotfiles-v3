...
**Updated:** Root callback SHALL declare only:
- `--output-format` — cross-cutting output serialization concern
- `--verbose`/`-v` — cross-cutting logging concern
- `--quiet`/`-q` — cross-cutting logging concern

All other flags SHALL be declared at leaf or sub-typer scope.
